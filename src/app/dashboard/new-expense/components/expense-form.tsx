...
import { CalendarIcon, PlusCircle, Trash2, Loader2, Check, ChevronRight, ChevronLeft } from 'lucide-react';
...
const STEPS = [
  { id: 1, label: 'Classificação', description: 'Plano de contas e descrição' },
  { id: 2, label: 'Valores', description: 'Valores, datas e pagamento' },
  { id: 3, label: 'Resultado', description: 'Centro de resultado e rateio' },
  { id: 4, label: 'Complemento', description: 'Fornecedor e observações' },
];
...
export default function ExpenseForm() {
  ...
  const progressPercent = useMemo(() => {
    const checks = [
      ...
      watchedAll.paymentMethod === 'installments'
        ? !!watchedAll.installmentType
        : !!watchedAll.dueDate,
      watchedAll.isApportioned
        ? (watchedAll.apportionments?.length ?? 0) > 0
        : !!watchedAll.resultCenter,
    ];
  ...
  const handleNextStep = async () => {
    const fieldsToValidate: any = {
      1: ['accountPlan', 'description'],
      2: ['totalValue', 'competenceDate', 'paymentMethod', 'dueDate', 'installments', 'installmentType', 'firstInstallmentDueDate'],
      3: ['isApportioned', 'resultCenter', 'apportionments'],
    };
    ...
    <Form {...form}>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 gap-8 lg:grid-cols-3 lg:items-start">
        <div className="space-y-8 lg:col-span-2">
          {currentStep === 1 && (
            <Card className={cn("transition-all duration-300 ring-primary/20", currentStep === 1 && "ring-2")}>
              <CardHeader>
          ...
          {currentStep === 2 && (
            <div className="space-y-8">
              <Card className={cn("transition-all duration-300 ring-primary/20", currentStep === 2 && "ring-2")}>
                <CardHeader>
                  <div className="flex items-center gap-3">
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary">
                      2
          ...
          {currentStep === 3 && (
            <Card className={cn("transition-all duration-300 ring-primary/20", currentStep === 3 && "ring-2")}>
              <CardHeader>
                <div className="flex items-center gap-3">
                  <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary">
                    3
          ...
          {currentStep === 4 && (
            <div className="space-y-8">
              <Card className={cn("transition-all duration-300 ring-primary/20", currentStep === 4 && "ring-2")}>
                <CardHeader>
                  <div className="flex items-center gap-3">
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary">
                      4
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea placeholder="Detalhes adicionais sobre a despesa..." className="min-h-[120px]" {...field} value={field.value ?? ''} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
